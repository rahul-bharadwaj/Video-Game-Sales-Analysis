...
```{r visdat, echo = TRUE, fig.cap='Visulaization of Data Types of the data', fig.height = 2, fig.width = 4}
...
```{r visguess, echo = TRUE, fig.cap='Data Type for each cell in dataset', fig.height = 3, fig.width = 4}
...
```{r vismiss, echo = TRUE, fig.cap='Missing Values Plot', fig.height = 5, fig.width = 5}
...
```{r viscor, echo = TRUE, fig.cap='Correlation Plot for variables', fig.height = 3, fig.width = 5}
...
```{r gathercor, echo = TRUE, fig.cap='Correlation between variables'}
...
```{r RegionSmooth, fig.cap='Smoothened curves for Regional Sales and Global Sales', fig.height = 2, fig.width = 4}
...
* The above plot is smoothened curve of all the observations and how the tend to change.

* The following correlation displays the above table as a plot with all the exact values.

```{r RegionCor, fig.cap='A plot of Correlation between Regional Sales and Global Sales', fig.height = 3, fig.width = 5}
...
* Let us consider some of the most famous game franchises and check what franchise sold most on what gaming platform. The considered franchises are Grand Theft Auto, FIFA, Pokemon, Mario, Need for Speed, and Call of Duty.

```{r AllTimeSales, fig.cap='All Time Global Sales (millions)', fig.width=5, fig.height=5}
...
EADensity <- vgsales %>% filter(str_detect(vgsales$Publisher, "Electronic Arts")) %>%
  ...
  colour = "blue") + xlab("EA Games GlobalSales") + ylab("Density")
```
...
ActivisionDensity <- vgsales %>% filter(str_detect(vgsales$Publisher, "Activision")) %>%
  ...
  colour = "orange4") + xlab("Activision Games GlobalSales") + ylab("Density")
```
...
UbiSoftDensity <- vgsales %>% filter(str_detect(vgsales$Publisher, "Ubisoft")) %>%
  ...
  colour = "green4") + xlab("Ubisoft Games GlobalSales") + ylab("Density")
```
...
SonyDensity <- vgsales %>% filter(str_detect(vgsales$Publisher, "Sony Computer Entertainment")) %>%
  ...
  colour = "red4") + xlab("Sony Games GlobalSales") + ylab("Density")
```
